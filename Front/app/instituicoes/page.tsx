...
import { instituicoesStorage, generateId } from "@/lib/local-storage"
...
export default function InstituicoesPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingInstituicao) {
      instituicoesStorage.update(editingInstituicao.id, formData)
    } else {
      instituicoesStorage.create({
        id: generateId(),
        ...formData,
      })
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-foreground text-balance">Instituições</h1>
          <p className="text-muted-foreground mt-1">Gerencie as instituições cadastradas</p>
        </div>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <DialogFooter>
                ...
                <Button type="submit">{editingInstituicao ? "Atualizar" : "Criar"}</Button>
              </DialogFooter>
            </form>
          </DialogContent>
        </Dialog>
              ...
              {filteredInstituicoes.length === 0 ? (
                ...
                filteredInstituicoes.map((instituicao) => (
                  <TableRow key={instituicao.id}>
                    ...
                    <TableCell>{instituicao.telefone}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => handleEdit(instituicao)}>
                          <Pencil className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(instituicao.id)}>
